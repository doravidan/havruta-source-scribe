-- Live shared chavruta study rooms: durable room state, per-user segment status,
-- and segment-scoped questions. Realtime subscriptions can listen to these rows;
-- WebRTC audio signaling uses Supabase broadcast channels and is not persisted.
...
GRANT ALL ON public.chavruta_study_sessions, public.chavruta_study_progress, public.chavruta_study_questions TO service_role;
...
CREATE POLICY "participants read study progress" ON public.chavruta_study_progress
  FOR SELECT TO authenticated USING (
    EXISTS (
      ...
      JOIN public.chavruta_matches m ON m.id = s.match_id
      WHERE s.id = session_id AND (m.requester_id = auth.uid() OR m.suggested_user_id = auth.uid())
    )
  );
