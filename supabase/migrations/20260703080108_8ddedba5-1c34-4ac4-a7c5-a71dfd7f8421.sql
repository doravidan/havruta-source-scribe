...
-- Admin-only RPCs to build the halfvec HNSW index via pg_cron (no HTTP timeout).
...
BEGIN
  ...
  -- One-shot job: build the index in a background worker, then unschedule itself.
  _cmd := format($cmd$
    DO $body$
    BEGIN
...
REVOKE ALL ON FUNCTION public.admin_start_hnsw_build(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_start_hnsw_build(text) TO authenticated;
...
REVOKE ALL ON FUNCTION public.admin_hnsw_build_status() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_hnsw_build_status() TO authenticated;
